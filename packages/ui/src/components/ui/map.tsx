...
function MarkerTooltip({ children, className, ...popupOptions }: MarkerTooltipProps) {
	...
	useEffect(() => {
		...
		// eslint-disable-next-line react-hooks/exhaustive-deps
	}, [map, container, marker.getElement, marker.getLngLat, tooltip.remove, tooltip.setDOMContent, tooltip.setLngLat]);
...
function MapControls({
	...
	return (
		<div className={cn("absolute z-10 flex flex-col gap-1.5", positionClasses[position], className)}>
			...
			{showLocate && (
				<ControlGroup>
					<ControlButton disabled={waitingForLocation} label="Find my location" onClick={handleLocate}>
						{waitingForLocation ? (
...
function MapPopup({
	...
	useEffect(() => {
		...
		// eslint-disable-next-line react-hooks/exhaustive-deps
	}, [map, container, popup.addTo, popup.isOpen, popup.off, popup.on, popup.remove, popup.setDOMContent]);
