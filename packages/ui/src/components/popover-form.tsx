"use client";

import { type ReactNode, type RefObject, useEffect, useRef } from "react";

import { ChevronUp, Loader } from "lucide-react";
import { AnimatePresence, motion } from "motion/react";

type PopoverFormProps = {
	open: boolean;
	setOpen: (open: boolean) => void;
	openChild?: ReactNode;
	successChild?: ReactNode;
	showSuccess: boolean;
	width?: string;
	height?: string;
	showCloseButton?: boolean;
	title: string;
};

export function PopoverForm({
	open,
	setOpen,
	openChild,
	showSuccess,
	successChild,
	width = "364px",
	height = "192px",
	title = "Feedback",
	showCloseButton = false,
}: PopoverFormProps) {
	const ref = useRef<HTMLDivElement>(null);
	useClickOutside(ref, () => setOpen(false));

	return (
		<div className="flex items-center justify-center" key={title}>
			<motion.button
				className="flex h-9 items-center border bg-white px-3 font-medium text-sm outline-none dark:bg-[#121212]"
				layoutId={`${title}-wrapper`}
				onClick={() => setOpen(true)}
				style={{ borderRadius: 8 }}
			>
				<motion.span layoutId={`${title}-title`}>{title}</motion.span>
			</motion.button>
			<AnimatePresence>
				{open && (
					<motion.div
						className="absolute overflow-hidden bg-muted p-1 shadow-[0_0_0_1px_rgba(0,0,0,0.08),0px_1px_2px_rgba(0,0,0,0.04)] outline-none"
						layoutId={`${title}-wrapper`}
						ref={ref}
						style={{ borderRadius: 10, width, height }}
					>
						<motion.span
							aria-hidden
							className="absolute top-[17px] left-4 text-muted-foreground text-sm data-success:text-transparent"
							data-success={showSuccess}
							layoutId={`${title}-title`}
						>
							{title}
						</motion.span>

						{showCloseButton && (
							<div className="absolute -top-[5px] left-1/2 z-20 flex h-[26px] w-[12px] -translate-x-1/2 transform items-center justify-center">
								<button
									aria-label="Close"
									className="absolute z-10 -mt-1 flex h-[6px] w-[10px] items-center justify-center rounded-full text-muted-foreground hover:text-foreground focus:outline-none"
									onClick={() => setOpen(false)}
								>
									<ChevronUp className="text-muted-foreground/80" />
								</button>

								<PopoverFormCutOutTopIcon />
							</div>
						)}

						<AnimatePresence mode="popLayout">
							{showSuccess ? (
								<motion.div
									animate={{ y: 0, opacity: 1, filter: "blur(0px)" }}
									className="flex h-full flex-col items-center justify-center"
									initial={{ y: -32, opacity: 0, filter: "blur(4px)" }}
									key="success"
									transition={{
										type: "spring" as const,
										duration: 0.4,
										bounce: 0,
									}}
								>
									{successChild || <PopoverFormSuccess />}
								</motion.div>
							) : (
								<motion.div
									className="z-20 h-full border bg-white dark:bg-[#121212]"
									exit={{ y: 8, opacity: 0, filter: "blur(4px)" }}
									key="open-child"
									style={{ borderRadius: 10 }}
									transition={{
										type: "spring" as const,
										duration: 0.4,
										bounce: 0,
									}}
								>
									{openChild}
								</motion.div>
							)}
						</AnimatePresence>
					</motion.div>
				)}
			</AnimatePresence>
		</div>
	);
}

export function PopoverFormButton({ loading, text = "submit" }: { loading: boolean; text: string }) {
	return (
		<button
			className="ml-auto flex h-6 w-26 items-center justify-center overflow-hidden rounded-md bg-linear-to-b from-primary/90 to-primary px-3 font-semibold text-primary-foreground text-xs shadow-[0_0_1px_1px_rgba(255,255,255,0.08)_inset,0_1px_1.5px_0_rgba(0,0,0,0.32),0_0_0_0.5px_#1a94ff]"
			type="submit"
		>
			<AnimatePresence initial={false} mode="popLayout">
				<motion.span
					animate={{ opacity: 1, y: 0 }}
					className="flex w-full items-center justify-center"
					exit={{ opacity: 0, y: 25 }}
					initial={{ opacity: 0, y: -25 }}
					key={`${loading}`}
					transition={{
						type: "spring",
						duration: 0.3,
						bounce: 0,
					}}
				>
					{loading ? <Loader className="size-3 animate-spin" /> : <span>{text}</span>}
				</motion.span>
			</AnimatePresence>
		</button>
	);
}

const useClickOutside = (
	ref: RefObject<HTMLElement | null>,
	handleOnClickOutside: (event: MouseEvent | TouchEvent) => void
) => {
	useEffect(() => {
		const listener = (event: MouseEvent | TouchEvent) => {
			if (!ref.current || ref.current.contains(event.target as Node)) {
				return;
			}
			handleOnClickOutside(event);
		};
		document.addEventListener("mousedown", listener);
		document.addEventListener("touchstart", listener);
		return () => {
			document.removeEventListener("mousedown", listener);
			document.removeEventListener("touchstart", listener);
		};
	}, [ref, handleOnClickOutside]);
};

export function PopoverFormSuccess({ title = "Success", description = "Thank you for your submission" }) {
	return (
		<>
			<svg
				className="-mt-1"
				fill="none"
				height="32"
				viewBox="0 0 32 32"
				width="32"
				xmlns="http://www.w3.org/2000/svg"
			>
				<path
					d="M27.6 16C27.6 17.5234 27.3 19.0318 26.717 20.4392C26.1341 21.8465 25.2796 23.1253 24.2025 24.2025C23.1253 25.2796 21.8465 26.1341 20.4392 26.717C19.0318 27.3 17.5234 27.6 16 27.6C14.4767 27.6 12.9683 27.3 11.5609 26.717C10.1535 26.1341 8.87475 25.2796 7.79759 24.2025C6.72043 23.1253 5.86598 21.8465 5.28302 20.4392C4.70007 19.0318 4.40002 17.5234 4.40002 16C4.40002 12.9235 5.62216 9.97301 7.79759 7.79759C9.97301 5.62216 12.9235 4.40002 16 4.40002C19.0765 4.40002 22.027 5.62216 24.2025 7.79759C26.3779 9.97301 27.6 12.9235 27.6 16Z"
					fill="#2090FF"
					fillOpacity="0.16"
				/>
				<path
					d="M12.1334 16.9667L15.0334 19.8667L19.8667 13.1M27.6 16C27.6 17.5234 27.3 19.0318 26.717 20.4392C26.1341 21.8465 25.2796 23.1253 24.2025 24.2025C23.1253 25.2796 21.8465 26.1341 20.4392 26.717C19.0318 27.3 17.5234 27.6 16 27.6C14.4767 27.6 12.9683 27.3 11.5609 26.717C10.1535 26.1341 8.87475 25.2796 7.79759 24.2025C6.72043 23.1253 5.86598 21.8465 5.28302 20.4392C4.70007 19.0318 4.40002 17.5234 4.40002 16C4.40002 12.9235 5.62216 9.97301 7.79759 7.79759C9.97301 5.62216 12.9235 4.40002 16 4.40002C19.0765 4.40002 22.027 5.62216 24.2025 7.79759C26.3779 9.97301 27.6 12.9235 27.6 16Z"
					stroke="#2090FF"
					strokeLinecap="round"
					strokeLinejoin="round"
					strokeWidth="2.4"
				/>
			</svg>
			<h3 className="mt-2 mb-1 font-medium text-primary text-sm">{title}</h3>
			<p className="mx-auto max-w-xs text-pretty text-center text-muted-foreground text-sm">{description}</p>
		</>
	);
}

export function PopoverFormSeparator({ width = 352, height = 2 }: { width?: number | string; height?: number }) {
	return (
		<svg
			className="absolute -top-px right-0 left-0"
			fill="none"
			height={height}
			viewBox="0 0 352 2"
			width={width}
			xmlns="http://www.w3.org/2000/svg"
		>
			<path className="stroke-border" d="M0 1H352" strokeDasharray="4 4" />
		</svg>
	);
}

function PopoverFormCutOutTopIcon({ width = 44, height = 30 }: { width?: number; height?: number }) {
	const aspectRatio = 6 / 12;
	const calculatedHeight = width * aspectRatio;
	const calculatedWidth = height / aspectRatio;

	const finalWidth = Math.min(width, calculatedWidth);
	const finalHeight = Math.min(height, calculatedHeight);

	return (
		<svg
			className="mt-px rotate-90"
			fill="none"
			height={finalHeight}
			preserveAspectRatio="none"
			viewBox="0 0 6 12"
			width={finalWidth}
			xmlns="http://www.w3.org/2000/svg"
		>
			<g clipPath="url(#clip0_2029_22)">
				<path
					className="fill-muted"
					d="M0 2C0.656613 2 1.30679 2.10346 1.91341 2.30448C2.52005 2.5055 3.07124 2.80014 3.53554 3.17157C3.99982 3.54301 4.36812 3.98396 4.6194 4.46927C4.87067 4.95457 5 5.47471 5 6C5 6.52529 4.87067 7.04543 4.6194 7.53073C4.36812 8.01604 3.99982 8.45699 3.53554 8.82843C3.07124 9.19986 2.52005 9.4945 1.91341 9.69552C1.30679 9.89654 0.656613 10 0 10V6V2Z"
				/>
				<path
					className="stroke-border"
					d="M1 12V10C2.06087 10 3.07828 9.57857 3.82843 8.82843C4.57857 8.07828 5 7.06087 5 6C5 4.93913 4.57857 3.92172 3.82843 3.17157C3.07828 2.42143 2.06087 2 1 2V0"
					strokeLinejoin="round"
					strokeWidth={0.6}
				/>
			</g>
			<defs>
				<clipPath id="clip0_2029_22">
					<rect fill="white" height={finalHeight} width={finalWidth} />
				</clipPath>
			</defs>
		</svg>
	);
}

export function PopoverFormCutOutLeftIcon() {
	return (
		<svg fill="none" height="12" viewBox="0 0 6 12" width="6" xmlns="http://www.w3.org/2000/svg">
			<g clipPath="url(#clip0_2029_22)">
				<path
					className="fill-muted"
					d="M0 2C0.656613 2 1.30679 2.10346 1.91341 2.30448C2.52005 2.5055 3.07124 2.80014 3.53554 3.17157C3.99982 3.54301 4.36812 3.98396 4.6194 4.46927C4.87067 4.95457 5 5.47471 5 6C5 6.52529 4.87067 7.04543 4.6194 7.53073C4.36812 8.01604 3.99982 8.45699 3.53554 8.82843C3.07124 9.19986 2.52005 9.4945 1.91341 9.69552C1.30679 9.89654 0.656613 10 0 10V6V2Z"
				/>
				<path
					className="stroke-border"
					d="M1 12V10C2.06087 10 3.07828 9.57857 3.82843 8.82843C4.57857 8.07828 5 7.06087 5 6C5 4.93913 4.57857 3.92172 3.82843 3.17157C3.07828 2.42143 2.06087 2 1 2V0"
					strokeLinejoin="round"
					strokeWidth="1"
				/>
			</g>
			<defs>
				<clipPath id="clip0_2029_22">
					<rect fill="white" height="12" width="6" />
				</clipPath>
			</defs>
		</svg>
	);
}

export function PopoverFormCutOutRightIcon() {
	return (
		<svg fill="none" height="12" viewBox="0 0 6 12" width="6" xmlns="http://www.w3.org/2000/svg">
			<g clipPath="url(#clip0_2029_22)">
				<path
					className="fill-muted"
					d="M0 2C0.656613 2 1.30679 2.10346 1.91341 2.30448C2.52005 2.5055 3.07124 2.80014 3.53554 3.17157C3.99982 3.54301 4.36812 3.98396 4.6194 4.46927C4.87067 4.95457 5 5.47471 5 6C5 6.52529 4.87067 7.04543 4.6194 7.53073C4.36812 8.01604 3.99982 8.45699 3.53554 8.82843C3.07124 9.19986 2.52005 9.4945 1.91341 9.69552C1.30679 9.89654 0.656613 10 0 10V6V2Z"
				/>
				<path
					className="stroke-border"
					d="M1 12V10C2.06087 10 3.07828 9.57857 3.82843 8.82843C4.57857 8.07828 5 7.06087 5 6C5 4.93913 4.57857 3.92172 3.82843 3.17157C3.07828 2.42143 2.06087 2 1 2V0"
					strokeLinejoin="round"
					strokeWidth="1"
				/>
			</g>
			<defs>
				<clipPath id="clip0_2029_22">
					<rect fill="white" height="12" width="6" />
				</clipPath>
			</defs>
		</svg>
	);
}

export default PopoverForm;
