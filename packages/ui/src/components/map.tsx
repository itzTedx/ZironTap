...
import { PlaceAutocomplete, type PlaceAutocompleteProps } from "@ziron/ui/components/place-autocomplete";
...
import type { MenuCheckboxItem } from "./menu";

// biome-ignore lint/suspicious/noExplicitAny: too much components
...
interface MapLayerGroupOption extends Pick<React.ComponentProps<typeof MenuCheckboxItem>, "disabled"> {
	name: string;
}
...
function MapTileLayer({
	...
	return <LeafletTileLayer attribution={resolvedAttribution} url={resolvedUrl} {...props} />;
}
			...
			<DropdownMenuContent align="end" className="z-1000">
				{showTileLayersDropdown && (
					<>
						...
						<DropdownMenuRadioGroup onValueChange={setSelectedTileLayer} value={selectedTileLayer}>
...
function MapLocateControl({
	...
	return (
		<MapControlContainer className={cn(position, className)}>
			<Button
				aria-label={
					isLocating ? "Locating..." : location ? "Stop location tracking" : "Start location tracking"
				}
				className="border"
				disabled={isLocating}
			...
			{location && <MapMarker icon={<MapLocatePulseIcon />} position={location} />}
		</MapControlContainer>
	);
}
